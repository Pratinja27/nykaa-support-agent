...
[![Render - Live Demo](https://img.shields.io/badge/Render-Live%20Demo-brightgreen?style=for-the-badge&logo=render)](https://nykaa-support-agent-1.onrender.com)

**Live Application:** https://nykaa-support-agent-1.onrender.com
...
Queries scoring below `0.35` automatically trigger the fallback refusal response:
...
#### 1. Query: *What is the return window for footwear?*
...
#### 2. Query: *How long does a COD refund take?*
...
#### 3. Query: *How long does standard delivery take?*
...
#### 4. Query: *Can I exchange my shoes for another size?*
...
#### 5. Query: *Can I cancel an order after it has shipped?*
...
**Recommendation:** We select **Fixed-Size Chunking** as the primary vector index for production deployment.

Both approaches achieve a perfect Recall@3 of 1.000, but Fixed-Size Chunking delivers higher Precision@3 (**0.467** vs **0.433**), reducing irrelevant context injected into the agent prompt.
...
$$\text{Escalation Score} = 0.6 \times \text{delayed signal} + 0.4 \times \left(\frac{\text{days since created}}{30}\right)$$
...
- **Nodes:**
  ...
  `guardrail_node` → `intent_router` → `rag_node` / `order_tool_node` → `escalation_node`
...
- **Groundedness Verification:** Checks context retrieval scores against the calibrated threshold (`0.35`). Queries scoring below the threshold (e.g., score `0.18`) trigger an automatic fallback response:

> "I am sorry, but I do not have enough verified information to answer this question accurately."
...
| **Similarity Search** | Vector Indexing | ChromaDB integration (`rag/index.py`) using Hugging Face embeddings for fast similarity search. | Recall@3: **1.000** across test sets |
...
- **Full Pipeline Integration:** Streamlit UI (`ui.py`) → FastAPI Backend (`api/app.py`) → LangGraph State Machine (`agent/graph.py`) → FastMCP Tools / ChromaDB RAG.
